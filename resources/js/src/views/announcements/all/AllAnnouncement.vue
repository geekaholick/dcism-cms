    ...
    <div style="height: inherit">
        ...
        <div v-if="isLoading">
            <section :class="itemView">
                <announcement-skeleton v-for="i in perPage" :key="i" />
            </section>
        </div>
        <div v-else>
            <div
                        ...
                        <div class="item-wrapper justify-content-start">
                            <div class="item-cost">
                                ...
                                >
                                    ...
                                    />
                                    {{
                                        announcement.comment_no >= 1000
                                            ? `${formatCommentCount(
                                                  announcement.comment_no
                                              )}k comments`
                                            : announcement.comment_no > 1
                                            ? `${announcement.comment_no} comments`
                                            : `${announcement.comment_no} comment`
                                    }}
                                </span>
                            </div>
                        </div>
    ...
    methods: {
        ...mapActions({
            ...
            getAnnouncementCount:
                announcementTypes.ACTION_GET_ANNOUNCEMENTS_COUNT,
        }),
        formatDate(date) {
            const months = [
                "January",
        ...
        updateList(params) {
            ...
            this.getAllAnnouncements(params)
                .then((res) => {
                    ...
                    for (
                        let offset = (params.page - 1) * params.items, i = 0;
                        i < res.length;
                        offset++, i++
                    ) {
